...
<script>
	...
	export default {
		...
		methods: {
			makeCall(event) {
				// this.standardService.start(true);
			...
			remoteLoaded(event) {
				// this.standardService.remoteVideo = event.object;

				this.standardService.remoteView = event.object;
			}, 
			localLoaded(event) {
				// this.standardService.localVideo = event.object;
				// this.standardService.localVideo.mirror = true;
